...
export default function SectionHero({ heading, search = true, offer = false, breadcrumbs }: Props) {
  const theme = useTheme();
  const router = useRouter();

  const boxRadius = { xs: 24, sm: 40 };

  const handleSelect = (event: SyntheticEvent, value: string | OptionType | null) => {
